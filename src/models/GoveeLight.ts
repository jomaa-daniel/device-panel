...
class GoveeLight extends Light {
    ...
    getLogData(): string {
        return JSON.stringify({ sku: this.sku, device: this.device, deviceName: this.deviceName });
    }
    ...
    async on(): Promise<void> {
        const payload: Payload = {
            sku: this.sku,
            device: this.device,
            capability: { type: CapabilityType.ON_OFF, instance: 'powerSwitch', value: 1 }
        }
        const requestId = randomUUID();

        try {
            await axios.post('https://openapi.api.govee.com/router/api/v1/device/control',
                { requestId, payload },
                {
    ...
    async off(): Promise<void> {
        const payload: Payload = {
            sku: this.sku,
            device: this.device,
            capability: { type: CapabilityType.ON_OFF, instance: 'powerSwitch', value: 0 }
        }
        const requestId = randomUUID();
        try {
            await axios.post('https://openapi.api.govee.com/router/api/v1/device/control',
    ...
    async setBrightness(brightness: Brightness): Promise<void> {
        const payload: Payload = {
            sku: this.sku,
            device: this.device,
            capability: { type: CapabilityType.RANGE, instance: 'brightness', value: brightness }
        }
        const requestId = randomUUID();
